...
function vigenereCipher(keyword, text, mode = 0) {
  let output = '';
  let oldToNewAlphasHashes = getOldToNewHashes(getCipherAlphabets(keyword), mode);
  let pos = 0; //Used to track current cipher alpha we're using.
...
function getCipherAlphabets(keyword) {
  let letters = keyword.toUpperCase().match(/[A-Z]/g); //ignore any other characters beside a-z.
...
function getOldToNewHashes(cipherAlphas, mode = 0) {
  let hashes = [];
  if (cipherAlphas) {
    for (let i = 0; i < cipherAlphas.length; i++) {
